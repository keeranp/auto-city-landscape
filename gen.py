...
def random_rectangle(img, depth):
    ...
    color = night_filter((randrange(255), randrange(255), randrange(255)))
...
def night_filter(color):
    B = 1
    G = .5
    R = .5

    return (color[0] * B, color[1] * G, color[2] * R)
...
for i in range(100):
    random_rectangle(img, 1.5)
    random_rectangle(img, 2.3)
